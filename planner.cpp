 ...
 */
millis_t Planner::time_fan_change  = 0 ; //2019.3.12 大熊添加的
...
{
  return time_fan_change;//返回风扇改变的时间 2019.3.25大熊修改，为了获取风扇修改的时间
...
void Planner::check_axes_activity() {
  ...
  unsigned char axis_active[NUM_AXIS] = { 0 },
    ...
    for (uint8_t i = 0; i < FAN_COUNT; i++) tail_fan_speed[i] = fanSpeeds[i];
		    ...
		    if(fan_pwm_last!=tail_fan_speed[0])//如果pwm发生了变化,并且大于之前的值
	    	{
	    	   //SERIAL_PROTOCOLLN(" fan pwm change"); //2019.3.25 大熊修改
               fan_pwm_last = tail_fan_speed[0];
			   time_fan_change = millis() ;//记录下修改的时间
		    }
  #endif
