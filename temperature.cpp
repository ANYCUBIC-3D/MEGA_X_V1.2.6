...
#define temp_per 0.3
float lasttemp[HOTENDS] = {0};
float yyyy(float newtemp,unsigned char e)
{
  float now=temp_per*newtemp + lasttemp[e] * (1-temp_per);
  lasttemp[e] = now;
  return now;
...
void Temperature::updateTemperaturesFromRawValues() {
  ...
  #endif
  HOTEND_LOOP() {
    current_temperature[e] = yyyy(Temperature::analog2temp(current_temperature_raw[e], e),e);
  }
...
void Temperature::isr() {
  ...
  if (temp_count >= OVERSAMPLENR) { // 10 * 16 * 1/(16000000/64/256)  = 164ms.
    ...
    static char BedMinTempCounter=0;
      ...
      {
        BedMinTempCounter++;
        if(BedMinTempCounter>20){BedMinTempCounter=0;max_temp_error(-1);}
        else BedMinTempCounter=0;
       }
      if (bed_minttemp_raw GEBED current_temperature_bed_raw && target_temperature_bed > 0.0f) 
      {
          BedMinTempCounter++;
          if(BedMinTempCounter>20){BedMinTempCounter=0;min_temp_error(-1);}
          else BedMinTempCounter=0;
      }
      else BedMinTempCounter=0;
    #endif
